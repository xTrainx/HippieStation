/obj/machinery/rnd/server
	...
	var/server_id = 0
	var/base_mining_income = 80             //HIPPIE CODE - CHanged from 2 to 80
	var/heat_gen = 100
	var/heating_power = 40000
	var/delay = 5   
	var/part_bonus = 1 //HIPIE CODE - value that point income is multiplied by, don't bother changing
	var/part_value = 0.333 //HIPIE CODE -the increase in part_bonus per tier of parts ABOVE stock parts, 2 with maximum parts
	var/temp_tolerance = 2.7 //HIPIE CODE - kelvin, minimum temperature is 2.7
	var/temp_penalty_coefficient = 0.5 //HIPIE CODE - Changes the penalty for high temperatures, 0.5 at 73k is about 2600 points/minute
...
/obj/machinery/rnd/server/RefreshParts()
	var/tot_rating = 0
	for(var/obj/item/stock_parts/SP in component_parts) //HIPIE CODE - changed from src to component_parts
...
/obj/machinery/rnd/server/proc/mine()
	. = base_mining_income
	var/penalty = max((get_env_temp() - temp_tolerance), 0) * temp_penalty_coefficient //HIPIE CODE -Change from / to * and temp_tolerance_low to temp_tolerance
	. = max(. - penalty, 0) * part_bonus //HIPIE CODE -added *partbones
